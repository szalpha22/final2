...
app.get('/', (req, res) => {
  ...
  const topEarners = db.prepare(`
    SELECT u.user_id, u.username, u.avatar, u.balance,
           COALESCE(SUM(p.amount), 0) as total_paid,
           (u.balance + COALESCE(SUM(p.amount), 0)) as lifetime_earnings,
           COUNT(DISTINCT s.id) as approved_clips,
           COALESCE(SUM(s.views), 0) as total_views
    FROM users u
    LEFT JOIN payouts p ON u.user_id = p.user_id AND p.status = 'approved'
    LEFT JOIN submissions s ON u.user_id = s.user_id AND s.status = 'approved'
    GROUP BY u.user_id
    HAVING lifetime_earnings > 0
    ORDER BY lifetime_earnings DESC
    LIMIT 5
  `).all();
  
  res.render('pages/home', { user: req.user, campaigns, topEarners });
});
